  ...
  return (
    <div className="min-h-screen pt-20 bg-site-tile bg-repeat bg-auto">
      <div 
        ...
        style={{ backgroundImage: `url(${course.feature_image_url})` }}
      >
        <div className="h-full w-full bg-black bg-opacity-60 flex items-center justify-center">
          <div className="max-w-4xl text-center px-4">
            <Link to="/courses" className="text-white opacity-80 hover:opacity-100 text-sm font-bold uppercase tracking-wider">
              &larr; Back to Courses
            </Link>
            <h1 
              className="text-4xl md:text-5xl font-serif font-bold uppercase tracking-wider text-white mt-4"
              style={{ WebkitTextStroke: '1px #B8860B', textShadow: '3px 3px 6px rgba(0,0,0,0.7)' }}
            >
              {course.title}
            </h1>
          </div>
      ...
      <div className="max-w-4xl mx-auto -mt-16">
        <div className="bg-white shadow-lg rounded-lg overflow-hidden p-8 md:p-12">
            ...
            {isAuthenticated && progress && progress.total_lessons > 0 && (
                <div className="mb-8 bg-gray-50 p-6 rounded-lg border border-gray-200">
                    ...
                    <div className="w-full bg-gray-300 rounded-full h-3">
                        <div className="bg-red-800 h-3 rounded-full transition-all duration-500 ease-out" style={{ width: `${progress.percent_complete}%` }}></div>
                    </div>
                ...
                <div className="space-y-4">
                    {lessons.map((lesson, index) => (
                        <Link 
                            ...
                            key={lesson.id}
                            className={`block p-6 border rounded-lg hover:shadow-md transition-shadow duration-200 ${lesson.is_completed ? 'border-green-200 bg-green-50' : 'border-gray-200 hover:border-red-300'}`}
                        >
                            <div className="flex items-center justify-between">
                                <div className="flex items-center space-x-4">
                                    <div className={`flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center font-bold ${lesson.is_completed ? 'bg-green-200 text-green-800' : 'bg-gray-200 text-gray-600'}`}>
                                        {index + 1}
                                    </div>
                                    <div>
                    ...
                    {lessons.length === 0 && (<p className="text-gray-500 italic">No lessons have been added to this course yet.</p>)}
                </div>
            </div>
        </div>
      </div>
